...
async function signInWithAccountPassword(account, password) {
  const supabase2 = ensureSupabase();
...
async function updateCurrentPassword(password) {
  ...
  const { error: profileError } = await supabase2.from("user_profiles").update({ must_change_password: false }).eq("id", context.user.id);
  if (profileError) {
    throw profileError;
  }
  return resolveAuthContext();
}
function getPostLoginTarget(context) {
