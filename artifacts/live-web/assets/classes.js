...
async function fetchClassesDirectory() {
  const supabase2 = ensureSupabase();
  return runQuery(
    supabase2.from("classes").select(CLASS_SELECT_FIELDS).order("created_at", { ascending: false })
  );
}
async function fetchClassMemberLinks() {
  const supabase2 = ensureSupabase();
  return runQuery(
    supabase2.from("class_students").select("id, class_id, student_id, joined_at, member_status").eq("member_status", "active")
  );
}
...
// assets/classes.js
var CLASS_STATUS_META = {
  active: "\u8FDB\u884C\u4E2D",
  draft: "\u8349\u7A3F",
  archived: "\u5DF2\u5F52\u6863"
};
var STUDENT_STATUS_META = {
  normal: "\u6B63\u5E38",
  temporary: "\u4E34\u65F6",
  pending_merge: "\u5F85\u5408\u5E76",
  merged: "\u5DF2\u5408\u5E76"
...
if (authContext) {
  const initClassesPage = function() {
      ...
      elements.classStudentSearchResults.innerHTML = state.searchResults.map(function(student) {
        ...
        <article class="teacher-search-card">
          <div class="teacher-search-card__text">
            ...
            <p>${escapeHtml(student.grade || "\u672A\u8BBE\u7F6E\u5E74\u7EA7")} \xB7 ${escapeHtml(student.student_code || "\u672A\u751F\u6210\u5B66\u53F7")} \xB7 ${escapeHtml(buildStudentStatusLabel(student.status))}</p>
    ...
    function renderClassDetail() {
      ...
      elements.classDetailMeta.textContent = `${campusName} \xB7 ${subjectName} \xB7 ${teacherName}`;
