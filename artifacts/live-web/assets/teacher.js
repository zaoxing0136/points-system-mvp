...
var WebSocketFactory = class {
  /**
   * Static-only utility 鈥?prevent instantiation.
   */
  constructor() {
  }
...
var StorageFileApi = class extends BaseApiClient {
  ...
  *
  * **Important:** Folder entries in the `folders` array only contain `name` and optionally `key` 鈥?  * they have no `id`, timestamps, or `metadata` fields. Full file metadata is only available
...
function getAuthDisplayName(context) {
  if (!context?.profile) {
    return "";
  }
  return context.profile.display_name || context.profile.teacher?.display_name || context.profile.teacher?.name || context.profile.phone || "\u672A\u547D\u540D\u8D26\u53F7";
}
...
async function fetchUserProfile(userId) {
  const supabase2 = ensureSupabase();
  const { data, error } = await supabase2.from("user_profiles").select(PROFILE_FIELDS).eq("id", userId).maybeSingle();
  if (error) {
    throw error;
  }
  return data || null;
...
function mountSessionActions(container, context) {
...
if (isFileMode) {
  ...
  const initTeacherPage = function() {
    ...
    function renderSpotlight(student) {
      ...
      const badgeSummaryLine = badgeSummary.nextLocked ? `\u4E0B\u4E00\u4E2A\uFF1A${badgeSummary.nextLocked.badge_name}\uFF0C\u518D ${badgeSummary.nextLocked.remaining_count} \u6B21\u201C${badgeSummary.nextLocked.event_label}\u201D` : badgeSummary.latestUnlocked ? `\u6700\u8FD1\u89E3\u9501\uFF1A${badgeSummary.latestUnlocked.badge_name}` : "\u8001\u5E08\u8BB0\u5F55\u884C\u4E3A\u540E\u4F1A\u81EA\u52A8\u7D2F\u8BA1\u5FBD\u7AE0\u8FDB\u5EA6";
    ...
    function renderStudentRecords() {
      ...
      elements.studentRecordList.innerHTML = state.studentRecords.map(function(record) {
        ...
        const detail = record.remark && record.remark !== record.rule_name_snapshot ? `<p class="teacher-record-detail">${escapeHtml(record.remark)}</p>` : "";
        return `
    ...
    async function handleAction(ruleId) {
      ...
      try {
        ...
        state.feedback = {
          ...
          note: `${CATEGORY_META[rule.category]?.label || rule.category} \xB7 ${rule.rule_name} +${rule.points} \u5206`,
          timestamp: Date.now()
        };
        renderAll();
        showToast(
    ...
    async function handleBadgeAction(badgeDefinitionId) {
      ...
      try {
        ...
        state.badgeFeedback = {
          ...
          unlockedJustNow,
          note: unlockedJustNow ? `${badgeDefinition.name} \u5DF2\u89E3\u9501\uFF0C${badgeDefinition.event_label} \u5DF2\u7D2F\u8BA1\u5230 ${eventCount} \u6B21` : `${badgeDefinition.event_label} \u5DF2\u8BB0\u5F55\uFF0C\u5F53\u524D ${eventCount} / ${threshold}`,
          timestamp: Date.now()
        };
        renderPanel();
