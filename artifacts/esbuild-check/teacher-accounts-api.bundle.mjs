...
// server/teacher-accounts.mjs
var ACCOUNT_EMAIL_DOMAIN = "accounts.points-mvp.local";
...
async function verifyAdminAccess(env, headers) {
  ...
  const { data: profile, error: profileError } = await anonClient.from("user_profiles").select("id, role, is_active").eq("id", userData.user.id).maybeSingle();
  if (profileError) {
...
async function findProfileByUserId(serviceClient, userId) {
  ...
  const { data, error } = await serviceClient.from("user_profiles").select("id, role, phone, display_name, teacher_id, is_active, must_change_password").eq("id", userId).maybeSingle();
  if (error) {
    throw createHttpError(500, error.message);
  }
  return data || null;
...
async function findProfileByTeacherId(serviceClient, teacherId) {
  ...
  const { data, error } = await serviceClient.from("user_profiles").select("id, role, phone, display_name, teacher_id, is_active, must_change_password").eq("teacher_id", teacherId).maybeSingle();
  if (error) {
    throw createHttpError(500, error.message);
  }
  return data || null;
...
function findAuthUser(users, options) {
  return users.find(function(user) {
    ...
    return user.id === options.userId || user.email === options.authEmail || loginName === options.loginName || options.phone && storedPhone === options.phone;
  }) || null;
}
async function createOrUpdateAuthUser(serviceClient, options) {
  ...
  if (options.existingUser) {
    const { data: data2, error: error2 } = await serviceClient.auth.admin.updateUserById(options.existingUser.id, payload);
    if (error2) {
      throw createHttpError(500, error2.message);
    }
    return { user: data2.user, mode: "updated" };
  }
...
async function migrateUserProfileId(serviceClient, existingProfileId, newUserId) {
  ...
  const { data, error } = await serviceClient.from("user_profiles").update({ id: newUserId }).eq("id", existingProfileId).select("id").single();
  if (error) {
    throw createHttpError(500, error.message);
  }
  return data.id;
...
async function upsertTeacherProfile(serviceClient, payload) {
  const { data, error } = await serviceClient.from("user_profiles").upsert(payload, { onConflict: "id" }).select("id, role, phone, display_name, teacher_id, is_active, must_change_password, created_at").single();
  if (error) {
    throw createHttpError(500, error.message);
  }
  return data;
...
async function getTeacherById(serviceClient, teacherId) {
  ...
  const { data, error } = await serviceClient.from("teachers").select("id, name, display_name, phone, campus_id, status").eq("id", teacherId).maybeSingle();
  if (error) {
    throw createHttpError(500, error.message);
  }
  if (!data) {
...
async function createTeacherRecord(serviceClient, displayName) {
  const { data, error } = await serviceClient.from("teachers").insert({
    name: displayName,
    ...
    campus_id: null,
    status: "active"
  }).select("id, name, display_name, phone, campus_id, status").single();
  if (error) {
    throw createHttpError(500, error.message);
  }
  return data;
...
async function syncTeacherDisplayName(serviceClient, teacher, displayName) {
  ...
  const { data, error } = await serviceClient.from("teachers").update({ display_name: displayName }).eq("id", teacher.id).select("id, name, display_name, phone, campus_id, status").single();
  if (error) {
    throw createHttpError(500, error.message);
  }
  return data;
...
async function listTeacherAccounts(env, headers) {
  ...
  const [profiles, users] = await Promise.all([
    serviceClient.from("user_profiles").select("id, role, phone, display_name, teacher_id, is_active, must_change_password, created_at, teacher:teacher_id ( id, name, display_name, phone, campus_id, status )").eq("role", "teacher").order("created_at", { ascending: false }),
...
async function resetTeacherPassword(env, headers, body) {
  ...
  const { error: profileError } = await serviceClient.from("user_profiles").update({ must_change_password: mustChangePassword }).eq("id", userId);
  if (profileError) {
    throw createHttpError(500, profileError.message);
  }
  return {
...
async function handleTeacherAccountsRequest({ method = "GET", headers = {}, body = null, env = process.env }) {
  try {
    if (method === "GET") {
    ...
    if (method === "POST") {
      ...
      if (action === "resetPassword") {
        const result2 = await resetTeacherPassword(env, headers, body || {});
        return { status: 200, body: { result: result2 } };
      }
