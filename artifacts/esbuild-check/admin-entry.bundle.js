...
var supabaseUrl, supabaseAnonKey, isSupabaseConfigured, supabase;
var init_supabase_client = __esm({
  "assets/supabase-client.js"() {
    ...
    supabase = isSupabaseConfigured ? createClient(supabaseUrl, supabaseAnonKey, {
      auth: {
        persistSession: true,
        autoRefreshToken: true,
        detectSessionInUrl: false
...
async function fetchAdminPointRules() {
  const supabase2 = ensureSupabase();
  return runQuery(
    supabase2.from("point_rules").select(ADMIN_POINT_RULE_FIELDS).order("category", { ascending: true }).order("is_common", { ascending: false }).order("sort_order", { ascending: true }).order("created_at", { ascending: true })
  );
}
async function upsertPointRules(rows) {
  const supabase2 = ensureSupabase();
  const payload = Array.isArray(rows) ? rows : [rows];
  return runQuery(
    supabase2.from("point_rules").upsert(payload, { onConflict: "id" }).select(ADMIN_POINT_RULE_FIELDS).order("category", { ascending: true }).order("is_common", { ascending: false }).order("sort_order", { ascending: true }).order("created_at", { ascending: true })
  );
}
...
async function upsertLevelTiers(rows) {
  const supabase2 = ensureSupabase();
  const payload = Array.isArray(rows) ? rows : [rows];
  return runQuery(
    supabase2.from("level_tiers").upsert(payload, { onConflict: "level_no" }).select(LEVEL_TIER_FIELDS).order("level_no", { ascending: true })
  );
}
...
async function upsertBadgeDefinitions(rows) {
  const supabase2 = ensureSupabase();
  const payload = Array.isArray(rows) ? rows : [rows];
  return runQuery(
    supabase2.from("badge_definitions").upsert(payload, { onConflict: "code" }).select(BADGE_DEFINITION_FIELDS).order("sort_order", { ascending: true }).order("created_at", { ascending: true }),
    "\u4FDD\u5B58\u5FBD\u7AE0\u89C4\u5219\u5931\u8D25"
  );
}
...
async function getTeacherAccountsAccessToken(supabase2) {
  let lastError = null;
...
async function requestTeacherAccountsApi(method, payload) {
  ...
  let lastError = null;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const accessToken = await getTeacherAccountsAccessToken(supabase2);
    ...
    const fallbackMessage = response.status === 404 ? "\u7EBF\u4E0A\u7F3A\u5C11 /api/admin/teacher-accounts \u63A5\u53E3\uFF0C\u8BF7\u786E\u8BA4\u5F53\u524D Vercel \u9879\u76EE\u5DF2\u5305\u542B api/admin/teacher-accounts.js \u5E76\u5B8C\u6210\u91CD\u65B0\u90E8\u7F72\u3002" : response.status >= 500 && /SUPABASE_(URL|ANON_KEY|SERVICE_ROLE_KEY)|缺少服务端 Supabase 环境变量/i.test(responseText) ? "Vercel \u670D\u52A1\u7AEF\u73AF\u5883\u53D8\u91CF\u4E0D\u5B8C\u6574\uFF0C\u8BF7\u8865\u9F50 SUPABASE_URL\u3001SUPABASE_ANON_KEY\u3001SUPABASE_SERVICE_ROLE_KEY\u3002" : `Teacher account API request failed (HTTP ${response.status}).`;
...
async function fetchTeacherAccountDirectory() {
  const result = await requestTeacherAccountsApi("GET");
  return result.accounts || [];
}
async function saveTeacherAccount(payload) {
...
async function resetTeacherAccountPassword(payload) {
  const result = await requestTeacherAccountsApi("POST", {
...
var ADMIN_POINT_RULE_FIELDS, LEVEL_TIER_FIELDS, BADGE_DEFINITION_FIELDS;
var init_supabase_service = __esm({
  "assets/supabase-service.js"() {
    init_supabase_client();
    ADMIN_POINT_RULE_FIELDS = "id, category, rule_name, points, sort_order, is_active, is_common, created_at";
    LEVEL_TIER_FIELDS = "id, level_no, level_name, threshold, is_active, created_at, updated_at";
    BADGE_DEFINITION_FIELDS = "id, code, name, description, event_label, icon_token, threshold, is_active, sort_order, created_at, updated_at";
  }
});

// assets/default-config.js
var DEFAULT_LEVEL_TIERS, DEFAULT_POINT_RULES, DEFAULT_BADGE_DEFINITIONS;
...
var init_avatar_library_manifest = __esm({
  "assets/avatars/avatar-library.manifest.js"() {
    avatar_library_manifest_default = {
      "version": 2,
      "implementation": "svg",
...
var EMPTY_LIBRARY, AVATAR_CATEGORIES, AVATAR_LIBRARY_LIST, AVATAR_CATEGORY_KEYS;
var init_avatar_library = __esm({
  "assets/avatar-library.js"() {
    init_avatar_library_manifest();
    EMPTY_LIBRARY = {};
    AVATAR_CATEGORIES = avatar_library_manifest_default?.categories || EMPTY_LIBRARY;
    AVATAR_LIBRARY_LIST = avatar_library_manifest_default?.list || [];
    AVATAR_CATEGORY_KEYS = Object.keys(AVATAR_CATEGORIES);
  }
});
...
var LEVEL_TIERS, CATEGORY_META;
var init_shared_ui = __esm({
  "assets/shared-ui.js"() {
    init_default_config();
    init_avatar_library();
    LEVEL_TIERS = DEFAULT_LEVEL_TIERS.map(function(tier) {
...
function initAdminPage() {
  ...
  function syncTeacherAccountPreview() {
    ...
    elements.teacherAccountFormHint.textContent = isEditing ? "\u4FEE\u6539\u8D26\u53F7\u540D\u3001\u5BC6\u7801\u6216\u542F\u505C\u72B6\u6001\u3002\u8001\u5E08\u4ECD\u7136\u4E0D\u7ED1\u5B9A\u56FA\u5B9A\u6821\u533A\u3002" : "\u53EA\u586B\u8001\u5E08\u540D\u79F0\u3001\u8D26\u53F7\u540D\u548C\u521D\u59CB\u5BC6\u7801\u3002\u4FDD\u5B58\u540E\u7CFB\u7EDF\u81EA\u52A8\u751F\u6210\u767B\u5F55\u90AE\u7BB1\u6620\u5C04\u3002";
  ...
  async function loadConfig() {
    ...
    try {
      const [
        ...
        fetchAdminPointRules().catch(function() {
          return [];
        }),
        fetchBadgeDefinitions({ activeOnly: false }).catch(function() {
          return [];
        }),
        fetchTeacherAccountsWithRetry().catch(function() {
          return [];
        }),
      ...
      renderAll();
      if (seeded.seeded) {
        refreshBadgeResultsInBackground(1e3);
...
var init_admin = __esm({
  "assets/admin.js"() {
    ...
    init_shared_ui();
    init_default_config();
    CATEGORY_ORDER = ["classroom", "homework", "project", "habits"];
...
if (authContext) {
  ...
  await Promise.resolve().then(() => (init_admin(), admin_exports));
}
